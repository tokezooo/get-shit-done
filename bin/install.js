...
function getManifestManagedCodexSkillNames(configDir, prefix = 'gsd-') {
...
function copyCommandsAsCodexSkills(srcDir, skillsDir, prefix, pathPrefix, runtime, managedSkillNames = []) {
  if (!fs.existsSync(srcDir)) {
    return;
  }
  ...
  // Remove only previously manifest-managed Codex skills to avoid deleting user-owned skills.
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode) {
    ...
    // Codex: remove only manifest-managed skills/gsd-* directories
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      const managedSkillNames = getManifestManagedCodexSkillNames(targetDir, 'gsd-');
      let skillCount = 0;
      for (const skillName of managedSkillNames) {
        const skillPath = path.join(skillsDir, skillName);
        if (fs.existsSync(skillPath)) {
...
function install(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode) {
    ...
    const gsdSrc = path.join(src, 'commands', 'gsd');
    const managedSkillNames = getManifestManagedCodexSkillNames(targetDir, 'gsd-');
    copyCommandsAsCodexSkills(gsdSrc, skillsDir, 'gsd', pathPrefix, runtime, managedSkillNames);
